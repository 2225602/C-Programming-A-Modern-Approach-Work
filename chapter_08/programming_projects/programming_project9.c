...
int main(void)
{
  ...
  while (remaining_walks > 0) {
	...
	case 0:  // GO UP
	  ...
	  break;
	case 1:  // GO DOWN
	  if (y_pos == 9 || board[y_pos + 1][x_pos] != '.') {
	  ...
	  break;
	case 2: // GO LEFT
	  if (x_pos == 0 || board[y_pos][x_pos - 1] != '.') {
	  ...
	  break;
	case 3: // GO RIGHT
	  if (x_pos == 9 || board[y_pos][x_pos + 1] != '.') {
